...
class Mesospim_BDV_Reader:
    '''
    A class for reading BDV files produced by the MesoSPIM.
    This enables multiscale access to each tile.
    The Class presents itself as a lazy numpy array and supports
    standard numpy slicing (readonly)
    '''
    _f = None
...
class Mesospim_BDV_Virtual_Montage(Mesospim_BDV_Reader):

    def __init__(self, path, subsample_z=False, blend='max'):
        self._relative_transforms_created = False
        self._multiscale_transforms_created = False
        self._subsample_z = subsample_z
        self.blend=blend
        super().__init__(path)
    ...
    def _get_subsample(self):
        if self._subsample_z:
            return (self.ResolutionSampling[self.ResolutionLevelLock][1],
                    *self.ResolutionSampling[self.ResolutionLevelLock][1:])
        else:
            return self.ResolutionSampling[self.ResolutionLevelLock]
    def set_resolution_lock(self, res_level: int):
        super().set_resolution_lock(res_level)
        if self._subsample_z:
            self.shape = (self.shape[0]//self._get_subsample()[1], *self.shape[1:])
        if not self._relative_transforms_created:
            self.get_transforms_relative_to_anchor()
            self._relative_transforms_created = True
        if not self._multiscale_transforms_created:
            self._collect_multiscale_transforms()
            self._multiscale_transforms_created = True
        self.compute_bounding_box()
    ...
    def _collect_multiscale_transforms(self):
        ...
        for res in range(self.ResolutionLevels):
            ...
            if self._subsample_z:
                sampling = (self.ResolutionSampling[res][1],*self.ResolutionSampling[res][1:])
            else:
                sampling = self.ResolutionSampling[res]

            for tile, trans in self.relative_transforms.items():
                self.multiscale_transforms[res][tile] = self.resample_translation(trans,sampling)
    ...
    def _get_slice(self,tile,resolution,slice):
        dset_path = self._get_dset_path(tile, resolution)
        with self as f:
            return f[dset_path][slice]
        ...
        for t in time_points:
            for c in channels:
                ...
                for tile in current_tiles:
                    ...
                    # Todo!
                    if self._subsample_z:
                        zsubsamp = self._get_subsample()[0]
                        t_n_r_bbox = (*t_n_r_bbox[0:3],t_n_r_bbox[3]*zsubsamp,*t_n_r_bbox[4:])
                    print(f'{t_n_r_bbox=}')
                    ...
                    from_tile = self._get_slice(tile,self.ResolutionLevelLock,t_n_r_slice)
                    if self._subsample_z:
                        avg_by = int(self._get_subsample()[0])
                        num_full_blocks = from_tile.shape[0] // avg_by
                        print(f'{avg_by=}')
                        from_tile = from_tile[:num_full_blocks*avg_by]
                        print(f'{from_tile.shape}')
                        # Reshape to group every 8 slices along z
                        reshaped = from_tile.reshape(avg_by,from_tile.shape[0] // avg_by,
                                                 from_tile.shape[1], from_tile.shape[2])

                        # Take the mean across the 8 slices
                        from_tile = reshaped.mean(axis=0)  # Resulting shape: (z//8, y, x)
                        from_tile = from_tile.astype('uint16')

                    print(f'From Tile Shape {from_tile.shape}')
                    if self.blend == 'max':
                        array[(t, c) + c_n_r_slice] = np.maximum(
                            from_tile,
                            array[(t, c) + c_n_r_slice]
                        )
                    elif self.blend == 'mean':
                        log_array[(t, c) + c_n_r_slice] += 1
                        array[(t, c) + c_n_r_slice] += from_tile
...
if __name__ == '__main__':
    ...
    bdv_file = '/CBI_FastStore/Acquire/MesoSPIM/alan-test-m1-h5/b1.1_stackingassay_Tile0_Ch488_Sh0.xml'
    bdv_file = Path(bdv_file)

    z = Mesospim_BDV_Virtual_Montage(bdv_file,subsample_z=True, blend='mean')
