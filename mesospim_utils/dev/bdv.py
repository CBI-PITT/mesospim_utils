...
class Mesospim_BDV_Reader:
    ...
    def compute_bounding_box(self, anchor_id=0):
        ...
        anchor_inv = np.linalg.inv(self.relative_transforms[anchor_id])

        all_transformed_points = []

        for setup_id, affine in self.relative_transforms.items():
    ...
    def __getitem__(self, item):
        ...
        for t in time_points:
            for c in channels:
                ...
                tiles_per_channel = self.TileNum//num_ch
                tiles_start = tiles_per_channel * c
                tiles_stop = tiles_start + tiles_per_channel

                for tile in range(self.TileNum)[tiles_start:tiles_stop]:
                    affine = self.relative_transforms[tile]
                    affine_inv = self.relative_transforms_inv[tile]
