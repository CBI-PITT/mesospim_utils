...
def decon_dir(dir_loc: str, refractive_index: float, out_dir: str=None, out_file_type: str='.tif', file_type: str='.btf',
    ...
    if run_slurm:
        ...
        if VERBOSE: print(f'SBATCH Job #: {job_number}')
    else:
    ...
    class rl(nn.Module):
        def __init__(self, psf, iterations=20, sigma=None):
            ...
            if sigma and sigma > 0:
                if VERBOSE: print('Preparing PSF with smoothing kernel')
        ...
        def forward(self, input_data):
            if sigma and sigma > 0:
                if VERBOSE: print('Preparing Input Data with smoothing kernel to reduce noise')
            ...
            if VERBOSE: print('Beginning deconvolution')
            with torch.no_grad():
                for i in range(self.iterations):
                    ...
                    if VERBOSE: print(f'Iteration {i+1} of {iterations}, Max: {estimate.max()}')
...
def richardson_lucy_3d(input_data, psf, iterations=50, sigma=None):
    ...
    if VERBOSE: print('Loading data onto GPU')
    input_data = input_data.to('cuda')
    psf = psf.to('cuda')

    if VERBOSE > 1: print(f'CUDA Input image dtype: {input_data.dtype}')
    if VERBOSE > 1: print(f'CUDA PSF dtype: {psf.dtype}')

    with torch.no_grad():
        # Noise reduction
        if sigma and sigma > 0:
            if VERBOSE: print('Smoothing data and PSF to reduce noise')
        ...
        for i in range(iterations):
            ...
            if i % 1 == 0:
                ...
                if VERBOSE: print(f"Iteration {i+1}/{iterations}, Time: {round(time_report(start_time=itter_start, to_print=False)/60, 2)} Sec")
...
def time_report(start_time=None, units='minutes', to_print=True):
    ...
    if start_time:
        ...
        if to_print:
            if VERBOSE: print(f'Total Time: {round(diff_time, 2)} {units}')
        return diff_time
...
def decon(file_location: Path, refractive_index: float, out_location: Path=None, resolution: tuple[float,float,float]=None,
    ...
    if VERBOSE: print(f'Input File: {file_location}')
    ...
    if VERBOSE: print('Generating PSF')
    ...
    if VERBOSE: print(f'Determining whether PSF should be trimmed')
    if VERBOSE: print(f'PSF input shape: {psf.shape}')
    psf = trim_psf(psf) #Reduce psf below threshold value set in constants
    if VERBOSE: print(f'PSF trimmed shape: {psf.shape}')
    ...
    if VERBOSE: print(f'--- PSF_SHAPE: {psf.shape}')
